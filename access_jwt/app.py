...
@jwt_app.route('/access-jwt', methods=['GET'])
def majorlogin_jwt():
...
@jwt_app.route('/token', methods=['GET'])
def oauth_guest():
    ...
    with jwt_app.test_request_context('/api/token', query_string=params):
        return majorlogin_jwt()

if __name__ == '__main__':
    jwt_app.run(host='0.0.0.0', port=1080, debug=False)